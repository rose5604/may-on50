...
try {
  ...
  console.error('MODEL_MAP bukan JSON valid. Contoh: {"llama3.3:70b-instruct-q4_K_M":"meta-llama/llama-3.3-70b-instruct"}');
  process.exit(1);
}

// API keys: multi-key (NOVITA_API_KEYS) atau single key (NOVITA_API_KEY)
const KEYS = String(process.env.NOVITA_API_KEYS || process.env.NOVITA_API_KEY || '')
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);

if (KEYS.length === 0) {
  console.error('Tidak ada API key. Isi NOVITA_API_KEYS (pisahkan koma) atau NOVITA_API_KEY.');
  process.exit(1);
}

// Limit per key (boleh 0 untuk menonaktifkan)
const RPM_PER_KEY = Math.max(0, Number(process.env.NOVITA_RPM_PER_KEY || '20')); // 0 = nonaktif
const RPS_PER_KEY = Math.max(0, Number(process.env.NOVITA_RPS_PER_KEY || '1'));  // 0 = nonaktif
...
async function callNovitaChatCompletions({ model, messages, stream = false, max_tokens }) {
  ...
  if (RL_MODE === 'pooled' && (RPM_PER_KEY > 0 || RPS_PER_KEY > 0)) {
    ...
    while (attempt <= MAX_RETRIES) {
      ...
      try {
        ...
        const resp = await axios.post(
          `${NOVITA_BASE}/v1/chat/completions`,
          requestBody,
    ...
    const e = new Error(`Gagal memanggil Novita setelah retry: ${lastErr?.message || 'unknown'}`);
  ...
  // MODE B: provider_only (failover instan, tanpa limiter lokal)
  {
    ...
    while (tried < maxAttempts) {
      ...
      try {
        ...
        const e = new Error(`Gagal memanggil Novita (fatal ${code || ''}): ${err?.message || 'unknown'}`);
        e.status = code || 500;
...
app.post('/api/generate', async (req, res) => {
  ...
  try {
    ...
    // Debug: Log actual tokens dari Novita
    const actualTokens = data?.usage?.completion_tokens || 0;
    console.log(`[TOKEN DEBUG] /api/generate - Requested: ${maxTokens}, Actual from Novita: ${actualTokens}`);
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    // Debug: Log actual tokens dari Novita
    const actualTokens = data?.usage?.completion_tokens || 0;
    console.log(`[TOKEN DEBUG] /api/chat - Requested: ${maxTokens}, Actual from Novita: ${actualTokens}`);
...
app.listen(PORT, () => {
  console.log(`Ollama->Novita proxy listening on :${PORT} | keys=${KEYS.length} | rl_mode=${RL_MODE}`);
});
